...
- **comment_id** - идентификационный номер комментария;
- **answer_id** - идентификационный номер ответа;
```                                                  
Производная таблица связи many-to-many сущности ответов и комментариев.
```
### Comment_question
...
- **question_id** - идентификационный номер вопроса;
```                                                  
Производная таблица связи many-to-many сущности вопросов и комментариев.
```
...
- **is_global** - флаг, показывающий является чат глобальным или нет;
```                                                  
Таблица групп чатов
```
...
- **chat_id** - идентификационный номер чата;
- **user_id** - идентификационный номер пользователя;
```                                                  
Производная таблица связи many-to-many сущности чата и пользователя.
```
...
- **lastRedactionDate** - дата последней редакции;
- **persistDate** - дата создания сообщения;
- **userSender** - пользователь, отправивший сообщение;
- **chat** - чат, в который отправлено сообщение;
```                                                  
Таблица сообщения
```
### Question_viewed
...
- **user** - пользователь, просмотревший сообщение;
- **question** - просмотренный вопрос;
- **localDateTime** - дата просмотра;
```                                                  
Таблица просмотра вопроса
```
...
- **id** - идентификационный номер чата;
- **userOne** - первый пользователь;
- **useTwo** - второй пользователь;
```                                                  
Таблица чата двух пользователей
```
### User_chat_pin
...
- **id** - идентификационный номер чата;
- **persistDateTime** - дата создания;
- **user** - пользователь;
- **chat** - чат;
```                                                  
Таблица принадлежности пользователя к чату
```
...
- **group_bookmark_id** - идентификационный номер группы;
- **bookmarks_id** - идентификационный номер закладки;
```                                                  
Производная таблица связи many-to-many сущности закладки группы и закладки.
```




[Схема](https://dbdiagram.io/d/649c03da02bd1c4a5e304fca)
...
Для тех кто будет писать контроллеры и расписывать их аннотациями Swagger, документацию по созданному вами api можно будет смотреть и проверять здесь: **http://localhost:8091/swagger-ui.html#/**
...
Запускаем проект в IDE. Переходим в Postman и выставляем следующие настройки:
...
**!Важно**  
Учитывайте, что в настройках секьюрити доступ к эндпоинту **"api/user/\*\*"** открыт только для пользователя
с ролью **User**, при попытке зайти туда админом вы получитe **403 Forbidden**.
