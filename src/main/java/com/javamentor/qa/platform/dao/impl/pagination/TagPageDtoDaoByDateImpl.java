...
public class TagPageDtoDaoByDateImpl implements PageDtoDao<TagViewDto> {
    ...
    public List<TagViewDto> getItems(PaginationData properties) {
        ...
        String hql = "SELECT NEW com.javamentor.qa.platform.models.dto.tag.TagViewDto(t.id, t.name, t.description," +
                     ...
                     "(SELECT COUNT(q.id) FROM Question q JOIN q.tags as qt WHERE (qt.id = t.id) AND cast (q.persistDateTime as date) >= current_date()-1)," +
                     "(SELECT COUNT(q.id) FROM Question q JOIN q.tags as qt WHERE (qt.id = t.id) AND cast (q.persistDateTime as date) >= current_date()-7))" +
                     "FROM Tag t ORDER BY t.persistDateTime";
